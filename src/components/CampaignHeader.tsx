    ...
    <header className="bg-dungeon-900 border-b-2 border-amber-700 px-4 md:px-6 py-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <button
            onClick={onMenuClick}
            className="md:hidden text-amber-600 hover:text-amber-400 transition-colors shrink-0"
          >
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          ...
          <button
            onClick={openStory}
            className="relative text-sm md:text-base font-semibold transition-colors hidden sm:block text-amber-400 hover:text-amber-300"
            style={{
          ...
          <div className="flex items-center gap-1 border-l border-amber-900 pl-2 md:pl-3">
            {isAdmin && (
              <div className="flex items-center gap-0.5 bg-amber-950/30 border border-amber-800/40 rounded-lg px-1" title="DM Tools">
                <button
        ...
        <div className="flex items-center gap-1.5 text-stone-400 text-xs">
          ...
          {countdown && (
            <>
              ...
              {campaign.nextSessionTime && <span className="text-amber-600">· {campaign.nextSessionTime}</span>}
            </>
          )}
        </div>
        <button
          onClick={openStory}
          className="relative text-xs font-semibold text-amber-400 transition-colors"
          style={{
            fontFamily: 'Cinzel, serif',
            ...(hasUnreadBlog ? { textShadow: '0 0 10px rgba(251,191,36,1), 0 0 24px rgba(251,191,36,0.6)' } : {}),
          }}
